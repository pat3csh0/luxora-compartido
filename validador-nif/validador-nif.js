...
(function() {
  ...
  function attach(input) {
    ...
    var validate = function() {
      ...
      if (result.cleaned && result.cleaned !== input.value.replace(/[\s.\-\/\\]/g, '').toUpperCase().trim()) {
        // Solo limpiar si realmente cambio algo sustancial
      }
      if (result.cleaned && result.cleaned !== cleanNif(input.value)) {
        // Nada extra que limpiar
      }
      // Siempre normalizar el formato en el campo
